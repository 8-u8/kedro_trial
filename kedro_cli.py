...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/pipeline_kedro_trial")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=pipeline_kedro_trial"]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "Kedro_pipeline_tutorial"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "pipeline_kedro_trial"
